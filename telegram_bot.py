...
class TelegramBot:
    ...
    def action(self):
        ...
        if self.incoming_message_text == '/hello':
            self.outgoing_message_text = "Hello {} {}!".format(
    ...
    def send_message(self):
        ...
        res = requests.get(TELEGRAM_SEND_MESSAGE_URL.format(
            self.chat_id, self.outgoing_message_text))

        print('message sent')
        return True if res.status_code == 200 else False

    def check_uniswap(self):
        """
Sends message to Telegram servers.
"""

        # res = requests.get(ETHERSCAN_TOKEN.format(self.chat_id, self.outgoing_message_text))
